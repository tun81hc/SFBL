...
uint32_t aa[4]= {0x01,0x02,0x03,0x04};
...
static HAL_StatusTypeDef ReceivePacket(uint8_t *p_data, uint32_t *p_length, uint32_t timeout)
{
  ...
  *p_length = 0;
  ...
  if (status == HAL_OK)
  {
    switch (char1)
    {
      case SOH:   //Start of header:  packet size = 128bytes
        packet_size = PACKET_SIZE;
        break;
      case STX:		//Start of header:  packet size = 1024bytes
...
COM_StatusTypeDef Ymodem_Receive ( uint32_t *p_size )
{
  ...
  while ((session_done == 0) && (result == COM_OK))
  {
    ...
    while ((file_done == 0) && (result == COM_OK))
    {
    	...
    	//FLASH_If_Erase(0x08008830);
    	//HAL_Delay(200);
    	//FLASH_If_Write(0x08008830, &file_done, 4);

    	//FLASH_If_Erase(0x08008000);

    	//Int2Str(file_1, file_done);
    	//Serial_PutString((uint8_t *)file_1);
    	//HAL_Delay(100);
